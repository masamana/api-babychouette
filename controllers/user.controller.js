...
const add = async (data) => {
    ...
    const [req, err] = await db.query("INSERT INTO users (email, password, name, first_name, tel, adress, roles) VALUES (?, ?, ?, ?, ?, ?, ?)", [data.email, hashedPassword, data.name, data.first_name, data.tel, data.adress, "user"]);
    if (!req) {
        return null;
    }
...
const update = async (id, data) => {
    ...
    if (!user) {
        ...
        const [req, err] = await db.query("UPDATE users SET email = ?, password = ?, name = ?, first_name = ?, tel = ?, adress = ? WHERE id = ? LIMIT 1", 
        [
